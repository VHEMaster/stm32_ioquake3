      ...
      Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
  ...
  if ( bpp <= 8 ) {     /* Palettized mode */
    int ncolors = 1<<bpp;
#ifdef DEBUG_PALETTE
    f_printf(stderr,"bpp=%d ncolors=%d\n",bpp,ncolors);
#endif
...
#ifdef DEBUG_PALETTE
      f_printf(stderr,"Rw=%d Rm=0x%02X\n",Rw,Rm);
#endif
      if(Gmask)
      {
        Gw=8-format->Gloss;
...
#ifdef DEBUG_PALETTE
      f_printf(stderr,"Gw=%d Gm=0x%02X\n",Gw,Gm);
#endif
      if(Bmask)
      {
        Bw=8-format->Bloss;
...
#ifdef DEBUG_PALETTE
      f_printf(stderr,"Bw=%d Bm=0x%02X\n",Bw,Bm);
#endif
#ifdef ENABLE_PALETTE_ALPHA
      if(Amask)
      {
...
# ifdef DEBUG_PALETTE
      f_printf(stderr,"Aw=%d Am=0x%02X\n",Aw,Am);
# endif
#endif
      for(i=0; i < ncolors; ++i) {
        int r,g,b;

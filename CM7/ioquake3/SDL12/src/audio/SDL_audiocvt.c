...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting to mono\n");
#endif
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting down to stereo\n");
#endif
	switch (format&0x8018) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting 6 down to quad\n");
#endif
	switch (format&0x8018) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting to stereo\n");
#endif
	if ( (format & 0xFF) == 16 ) {
		Uint16 *src, *dst;
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting stereo to surround\n");
#endif
	switch (format&0x8018) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting stereo to quad\n");
#endif
	switch (format&0x8018) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting to 16-bit LSB\n");
#endif
	src = cvt->buf+cvt->len_cvt;
	dst = cvt->buf+cvt->len_cvt*2;
	for ( i=cvt->len_cvt; i; --i ) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting to 16-bit MSB\n");
#endif
	src = cvt->buf+cvt->len_cvt;
	dst = cvt->buf+cvt->len_cvt*2;
	for ( i=cvt->len_cvt; i; --i ) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting to 8-bit\n");
#endif
	src = cvt->buf;
	dst = cvt->buf;
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting audio signedness\n");
#endif
	data = cvt->buf;
	if ( (format & 0xFF) == 16 ) {
		if ( (format & 0x1000) != 0x1000 ) { /* Little endian */
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting audio endianness\n");
#endif
	data = cvt->buf;
	for ( i=cvt->len_cvt/2; i; --i ) {
		tmp = data[0];
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting audio rate * 2\n");
#endif
	src = cvt->buf+cvt->len_cvt;
	dst = cvt->buf+cvt->len_cvt*2;
	switch (format & 0xFF) {
...
#ifdef DEBUG_CONVERT
	f_printf(stderr, "Converting audio rate * %4.4f\n", 1.0/cvt->rate_incr);
#endif

...
static void DMA_WaitAudio(_THIS)
{
	...
	if ( frame_ticks ) {
		...
		timeout.tv_usec = 0;
#ifdef DEBUG_AUDIO
		f_printf(stderr, "Waiting for audio to get ready\n");
#endif
		if ( select(audio_fd+1, NULL, &fdset, NULL, &timeout) <= 0 ) {
			...
			*/
			...
			audio_fd = -1;
#ifdef DEBUG_AUDIO
			f_printf(stderr, "Done disabling audio\n");
#endif
#endif /* AUDIO_OSPACE_HACK */
		}
#ifdef DEBUG_AUDIO
		f_printf(stderr, "Ready!\n");
#endif
	}
}
...
static int DMA_OpenAudio(_THIS, SDL_AudioSpec *spec)
{
	...
	for ( test_format = SDL_FirstAudioFormat(spec->format);
						! format && test_format; ) {
#ifdef DEBUG_AUDIO
		f_printf(stderr, "Trying format 0x%4.4x\n", test_format);
#endif
		switch ( test_format ) {
			case AUDIO_U8:
				if ( value & AFMT_U8 ) {

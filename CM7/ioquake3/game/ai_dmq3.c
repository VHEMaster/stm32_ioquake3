...
int BotPointAreaNum(vec3_t origin) {
	...
	areanum = G_trap_AAS_PointAreaNum(origin);
	if (areanum) return areanum;
	VectorCopy(origin, end);
	end[2] += 10;
	numareas = G_trap_AAS_TraceAreas(origin, end, areas, NULL, 10);
...
char *ClientName(int client, char *name, int size) {
	...
	G_trap_GetConfigstring(CS_PLAYERS+client, buf, sizeof(buf));
...
int ClientFromName(char *name) {
	...
	if (!maxclients)
		maxclients = G_trap_Cvar_VariableIntegerValue("sv_maxclients");
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		G_trap_GetConfigstring(CS_PLAYERS+i, buf, sizeof(buf));
		Q_CleanStr( buf );
...
void BotChooseWeapon(bot_state_t *bs) {
	...
	if (bs->cur_ps.weaponstate == WEAPON_RAISING ||
		...
		G_trap_EA_SelectWeapon(bs->client, bs->weaponnum);
	}
	else {
		newweaponnum = G_trap_BotChooseBestFightWeapon(bs->ws, bs->inventory);
...
void BotSetupForMovement(bot_state_t *bs) {
	...
	//
	G_trap_BotInitMoveState(bs->ms, &initmove);
}
		...
		if (offence) {
			if (!(bs->teamtaskpreference & TEAMTP_ATTACKER)) {
				...
				if (BotTeamLeader(bs)) {
					...
					//G_trap_BotEnterChat(bs->cs, leader, CHAT_TELL);
				}
...
qboolean BotIsObserver(bot_state_t *bs) {
	...
	G_trap_GetConfigstring(CS_PLAYERS+bs->client, buf, sizeof(buf));
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
	...
	feet[2] -= 23;
	return (G_trap_AAS_PointContents(feet) & (CONTENTS_LAVA|CONTENTS_SLIME));
}
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	rocketjumper = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_WEAPONJUMPING, 0, 1);
...
void BotGoCamp(bot_state_t *bs, bot_goal_t *goal) {
	...
	//get the team goal time
	camper = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CAMPER, 0, 1);
...
int BotWantsToCamp(bot_state_t *bs) {
	...
	for (cs = G_trap_BotGetNextCampSpotGoal(0, &goal); cs; cs = G_trap_BotGetNextCampSpotGoal(cs, &goal)) {
		traveltime = G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal.areanum, TFL_DEFAULT);
...
void BotDontAvoid(bot_state_t *bs, char *itemname) {
	...
	num = G_trap_BotGetLevelItemGoal(-1, itemname, &goal);
	while(num >= 0) {
		G_trap_BotRemoveFromAvoidGoals(bs->gs, goal.number);
		num = G_trap_BotGetLevelItemGoal(num, itemname, &goal);
	}
}
...
void BotRoamGoal(bot_state_t *bs, vec3_t goal) {
	...
	for (i = 0; i < 10; i++) {
		...
		if (len > 200) {
			...
			if (!trace.startsolid) {
				trace.endpos[2]++;
				pc = G_trap_PointContents(trace.endpos, bs->entitynum);
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	if (bs->attackchase_time > FloatTime()) {
		...
		G_trap_BotMoveToGoal(&moveresult, bs->ms, &goal, tfl);
		return moveresult;
	}
	//
	memset(&moveresult, 0, sizeof(bot_moveresult_t));
	//
	attack_skill = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_ATTACK_SKILL, 0, 1);
	jumper = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_JUMPER, 0, 1);
	croucher = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CROUCHER, 0, 1);
	...
	if (attack_skill <= 0.4) {
		...
		if (dist > attack_dist + attack_range) {
			if (G_trap_BotMoveInDirection(bs->ms, forward, 400, movetype)) return moveresult;
		}
		if (dist < attack_dist - attack_range) {
			if (G_trap_BotMoveInDirection(bs->ms, backward, 400, movetype)) return moveresult;
		}
		return moveresult;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (G_trap_BotMoveInDirection(bs->ms, sideward, 400, movetype))
			return moveresult;
...
int BotSameTeam(bot_state_t *bs, int entnum) {
	...
	if ( gametype >= GT_TEAM ) {
		G_trap_GetConfigstring(CS_PLAYERS+bs->client, info1, sizeof(info1));
		G_trap_GetConfigstring(CS_PLAYERS+entnum, info2, sizeof(info2));
		//
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent) {
	...
	//
	pc = G_trap_AAS_PointContents(eye);
	...
	for (i = 0; i < 3; i++) {
		...
		if (G_trap_AAS_PointContents(middle) & (CONTENTS_LAVA|CONTENTS_SLIME|CONTENTS_WATER)) {
		...
		if (trace.fraction >= 1 || trace.ent == hitent) {
			...
			otherinfog = (G_trap_AAS_PointContents(middle) & CONTENTS_FOG);
			if (infog && otherinfog) {
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	alertness = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_ALERTNESS, 0, 1);
	easyfragger = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_EASY_FRAGGER, 0, 1);
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	//
	aim_skill = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL, 0, 1);
	aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY, 0, 1);
	//
	if (aim_skill > 0.95) {
		//don't aim too early
		reactiontime = 0.5 * G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1);
		if (bs->enemysight_time > FloatTime() - reactiontime) return;
		if (bs->teleport_time > FloatTime() - reactiontime) return;
	}

	//get the weapon information
	G_trap_BotGetWeaponInfo(bs->ws, bs->weaponnum, &wi);
	//get the weapon specific aim accuracy and or aim skill
	if (wi.number == WP_MACHINEGUN) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0, 1);
	}
	else if (wi.number == WP_SHOTGUN) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0, 1);
	}
	else if (wi.number == WP_GRENADE_LAUNCHER) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADELAUNCHER, 0, 1);
		aim_skill = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_GRENADELAUNCHER, 0, 1);
	}
	else if (wi.number == WP_ROCKET_LAUNCHER) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_ROCKETLAUNCHER, 0, 1);
		aim_skill = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_ROCKETLAUNCHER, 0, 1);
	}
	else if (wi.number == WP_LIGHTNING) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_LIGHTNING, 0, 1);
	}
	else if (wi.number == WP_RAILGUN) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, 0, 1);
	}
	else if (wi.number == WP_PLASMAGUN) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_PLASMAGUN, 0, 1);
		aim_skill = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_PLASMAGUN, 0, 1);
	}
	else if (wi.number == WP_BFG) {
		aim_accuracy = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_BFG10K, 0, 1);
		aim_skill = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_BFG10K, 0, 1);
	}
	//
	...
	if (enemyvisible) {
		...
		if (wi.speed) {
			...
			if (!(dist > 100 && VectorLengthSquared(dir) < Square(32))) {
				...
				if (aim_skill > 0.8 &&
					...
					G_trap_AAS_PredictClientMovement(&move, bs->enemy, origin,
		...
		if (aim_skill > 0.5) {
			...
			if (wi.number == WP_BFG ||
				...
				VectorSet(goal.maxs, 8, 8, 8);
				//
				if (G_trap_BotPredictVisiblePosition(bs->lastenemyorigin, bs->lastenemyareanum, &goal, TFL_DEFAULT, target)) {
					VectorSubtract(target, bs->eye, dir);
	...
	if (bot_challenge.integer) {
		...
		if (aim_accuracy > 0.9 && bs->enemysight_time < FloatTime() - 1) {
			...
			G_trap_EA_View(bs->client, bs->viewangles);
		}
	}
}
...
void BotCheckAttack(bot_state_t *bs) {
	...
	//
	reactiontime = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1);
	if (bs->enemysight_time > FloatTime() - reactiontime) return;
	if (bs->teleport_time > FloatTime() - reactiontime) return;
	//if changing weapons
	...
	firethrottle = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_FIRETHROTTLE, 0, 1);
...
void BotMapScripts(bot_state_t *bs) {
	...
	G_trap_GetServerinfo(info, sizeof(info));
...
int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
	...
	G_trap_AAS_ValueForBSPEpairKey(bspent, "model", model, sizeof(model));
	if (!*model)
		return qfalse;
	...
	G_trap_AAS_FloatForBSPEpairKey(bspent, "lip", &lip);
	if (!lip) lip = 4;
	//get the move direction from the angle
	G_trap_AAS_FloatForBSPEpairKey(bspent, "angle", &angle);
	...
	dist *= 0.5;
	//
	G_trap_AAS_FloatForBSPEpairKey(bspent, "health", &health);
	...
	if (health) {
		...
		if (bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum) {
			...
			G_trap_AAS_PresenceTypeBoundingBox(PRESENCE_CROUCH, bboxmins, bboxmaxs);
			for (i = 0; i < 3; i++) {
			...
			numareas = G_trap_AAS_TraceAreas(start, end, areas, points, 10);
			//
			for (i = numareas-1; i >= 0; i--) {
				if (G_trap_AAS_AreaReachability(areas[i])) {
					break;
				}
			}
			if (i < 0) {
		...
		numareas = G_trap_AAS_TraceAreas(start, end, areas, NULL, 10);
		//
...
void BotEnableActivateGoalAreas(bot_activategoal_t *activategoal, int enable) {
	...
	for (i = 0; i < activategoal->numareas; i++)
		G_trap_AAS_EnableRoutingArea( activategoal->areas[i], enable );
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
	...
	for (ent = G_trap_AAS_NextBSPEntity(0); ent; ent = G_trap_AAS_NextBSPEntity(ent)) {
		if (!G_trap_AAS_ValueForBSPEpairKey(ent, "model", tmpmodel, sizeof(tmpmodel))) continue;
	...
	G_trap_AAS_ValueForBSPEpairKey(ent, "classname", classname, sizeof(classname));
	if (!*classname) {
	...
	if (!strcmp(classname, "func_door")) {
		if (G_trap_AAS_FloatForBSPEpairKey(ent, "health", &health)) {
		...
		//
		G_trap_AAS_IntForBSPEpairKey(ent, "spawnflags", &spawnflags);
		// if the door starts open then just wait for the door to return
		if ( spawnflags & 1 )
			return 0;
		//get the door origin
		if (!G_trap_AAS_VectorForBSPEpairKey(ent, "origin", origin)) {
			VectorClear(origin);
		}
		...
		G_trap_AAS_ValueForBSPEpairKey(ent, "model", model, sizeof(model));
		if (*model) {
			modelindex = atoi(model+1);
			if (modelindex) {
				VectorClear(angles);
				...
				//
				numareas = G_trap_AAS_BBoxAreas(absmins, absmaxs, areas, MAX_ACTIVATEAREAS*2);
				...
				for (i = 0; i < numareas; i++) {
					...
					G_trap_AAS_AreaInfo(areas[i], &areainfo);
	...
	if (!G_trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[0], sizeof(targetname[0]))) {
	...
	cur_entities[0] = G_trap_AAS_NextBSPEntity(0);
	for (i = 0; i >= 0 && i < 10;) {
		for (ent = cur_entities[i]; ent; ent = G_trap_AAS_NextBSPEntity(ent)) {
			if (!G_trap_AAS_ValueForBSPEpairKey(ent, "target", target, sizeof(target))) continue;
			if (!strcmp(targetname[i], target)) {
				cur_entities[i] = G_trap_AAS_NextBSPEntity(ent);
				break;
			}
		}
		if (!ent) {
		...
		if (!G_trap_AAS_ValueForBSPEpairKey(ent, "classname", classname, sizeof(classname))) {
			if (bot_developer.integer) {
		...
		if (!strcmp(classname, "func_button")) {
			...
			if ( G_trap_AAS_AreaReachability(bs->areanum) ) {
				// disable all areas the blocking entity is in
				BotEnableActivateGoalAreas( activategoal, qfalse );
				//
				t = G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, activategoal->goal.areanum, bs->tfl);
		...
		// the actual button or trigger might be linked through a target_relay or target_delay
			...
			if (G_trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname[i+1], sizeof(targetname[0]))) {
				i++;
				cur_entities[i] = G_trap_AAS_NextBSPEntity(0);
			}
		}
	}
#ifdef OBSTACLEDEBUG
...
void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal, int bspent) {
	...
	G_trap_AAS_ValueForBSPEpairKey(bspent, "classname", classname, sizeof(classname));
...
void BotRandomMove(bot_state_t *bs, bot_moveresult_t *moveresult) {
	...
	G_trap_BotMoveInDirection(bs->ms, dir, 400, MOVE_WALK);
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
	...
	if (activate && entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex) {
		...
		if (bspent) {
			...
			if (!(moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) &&
				G_trap_AAS_AreaReachability(bs->areanum))
				return;
		}
		else {
			// enable any routing areas that were disabled
	...
	if (movetype != MOVE_CROUCH || !G_trap_BotMoveInDirection(bs->ms, hordir, 400, movetype)) {
		// perform the movement
		if (!G_trap_BotMoveInDirection(bs->ms, sideward, 400, movetype)) {
			...
			G_trap_BotMoveInDirection(bs->ms, sideward, 400, movetype);
		}
	}
	//
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
	...
	G_trap_AAS_PredictRoute(&route, bs->areanum, bs->origin,
...
void BotCheckConsoleMessages(bot_state_t *bs) {
	...
	//
	while((handle = G_trap_BotNextConsoleMessage(bs->cs, &m)) != 0) {
		//if the chat state is flooded with messages the bot will read them quickly
		if (G_trap_BotNumConsoleMessages(bs->cs) < 10) {
		...
		if (!BotMatchMessage(bs, m.message)) {
			...
			if (m.type == CMS_CHAT && !bot_nochat.integer) {
				//
				if (!G_trap_BotFindMatch(m.message, &match, MTCONTEXT_REPLYCHAT)) {
					G_trap_BotRemoveConsoleMessage(bs->cs, handle);
					continue;
				}
				...
				//
				G_trap_BotMatchVariable(&match, NETNAME, netname, sizeof(netname));
				G_trap_BotMatchVariable(&match, MESSAGE, message, sizeof(message));
				...
				//unify the message
				G_trap_UnifyWhiteSpaces(message);
				//
				G_trap_Cvar_Update(&bot_testrchat);
				if (bot_testrchat.integer) {
					//
					G_trap_BotLibVarSet("bot_testrchat", "1");
					//if bot replies with a chat message
					if (G_trap_BotReplyChat(bs->cs, message, context, CONTEXT_REPLY,
															NULL, NULL,
															NULL, NULL,
															NULL, NULL,
															botname, netname)) {
				...
				//if at a valid chat position and not chatting already and not in teamplay
					...
					chat_reply = G_trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_REPLY, 0, 1);
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	G_trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 160, AVOID_ALWAYS);
}
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state) {
	...
	switch(event) {
		...
		case EV_GLOBAL_SOUND:
		{
			...
			G_trap_GetConfigstring(CS_SOUNDS + state->eventParm, buf, sizeof(buf));
			/*
...
void BotCheckSnapshot(bot_state_t *bs) {
	...
	G_trap_BotAddAvoidSpot(bs->ms, vec3_origin, 0, AVOID_CLEAR);
	//reset kamikaze body
	bs->kamikazebody = 0;
...
void BotCheckAir(bot_state_t *bs) {
	if (bs->inventory[INVENTORY_ENVIRONMENTSUIT] <= 0) {
		if (G_trap_AAS_PointContents(bs->eye) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)) {
			return;
		}
	}
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
	...
	if (bs->altroutegoal.areanum) {
		...
		t = G_trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->altroutegoal.areanum, bs->tfl);
		if (t && t < 20) {
...
#ifdef MISSIONPACK
	if (gametype == GT_CTF) {
		if (G_trap_BotGetLevelItemGoal(-1, "Neutral Flag", &ctf_neutralflag) < 0)
			BotAI_Print(PRT_WARNING, "no alt routes without Neutral Flag\n");
		if (ctf_neutralflag.areanum) {
			//
			red_numaltroutegoals = G_trap_AAS_AlternativeRouteGoals(
			...
			blue_numaltroutegoals = G_trap_AAS_AlternativeRouteGoals(
										ctf_neutralflag.origin, ctf_neutralflag.areanum,
		...
		if (G_trap_BotGetLevelItemGoal(-1, "Neutral Obelisk", &neutralobelisk) < 0)
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (bs->setupcount > 0) {
		...
		G_trap_Characteristic_String(bs->character, CHARACTERISTIC_GENDER, gender, sizeof(gender));
		//set the bot gender
		G_trap_GetUserinfo(bs->client, userinfo, sizeof(userinfo));
		...
		if ( !bs->map_restart && g_gametype.integer != GT_TOURNAMENT ) {
			...
			G_trap_EA_Command(bs->client, buf);
		}
		//set the chat gender
		if (gender[0] == 'm') G_trap_BotSetChatGender(bs->cs, CHAT_GENDERMALE);
		else if (gender[0] == 'f')  G_trap_BotSetChatGender(bs->cs, CHAT_GENDERFEMALE);
		else  G_trap_BotSetChatGender(bs->cs, CHAT_GENDERLESS);
		//set the chat name
		ClientName(bs->client, name, sizeof(name));
		G_trap_BotSetChatName(bs->cs, name, bs->client);
		//
	...
	if (i >= MAX_NODESWITCHES) {
		G_trap_BotDumpGoalStack(bs->gs);
		G_trap_BotDumpAvoidGoals(bs->gs);
...
int BotGoalForBSPEntity( char *classname, bot_goal_t *goal ) {
	...
	for (ent = G_trap_AAS_NextBSPEntity(0); ent; ent = G_trap_AAS_NextBSPEntity(ent)) {
		if (!G_trap_AAS_ValueForBSPEpairKey(ent, "classname", value, sizeof(value)))
			continue;
...
void BotSetupDeathmatchAI(void) {
	...
	gametype = G_trap_Cvar_VariableIntegerValue("g_gametype");
	maxclients = G_trap_Cvar_VariableIntegerValue("sv_maxclients");
	...
	//
	if (gametype == GT_CTF) {
		if (G_trap_BotGetLevelItemGoal(-1, "Red Flag", &ctf_redflag) < 0)
			BotAI_Print(PRT_WARNING, "CTF without Red Flag\n");
		if (G_trap_BotGetLevelItemGoal(-1, "Blue Flag", &ctf_blueflag) < 0)
		...
		if (G_trap_BotGetLevelItemGoal(-1, "Red Obelisk", &redobelisk) < 0)
			BotAI_Print(PRT_WARNING, "Obelisk without red obelisk\n");
		BotSetEntityNumForGoal(&redobelisk, "team_redobelisk");
		if (G_trap_BotGetLevelItemGoal(-1, "Blue Obelisk", &blueobelisk) < 0)
	...
	max_bspmodelindex = 0;
	for (ent = G_trap_AAS_NextBSPEntity(0); ent; ent = G_trap_AAS_NextBSPEntity(ent)) {
		if (!G_trap_AAS_ValueForBSPEpairKey(ent, "model", model, sizeof(model))) continue;
		if (model[0] == '*') {

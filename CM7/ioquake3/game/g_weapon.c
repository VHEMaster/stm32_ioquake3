...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	G_trap_Trace (&tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT);
...
void Bullet_Fire (gentity_t *ent, float spread, int damage ) {
	...
	for (i = 0; i < 10; i++) {
		...
		G_trap_Trace (&tr, muzzle, NULL, NULL, end, passent, MASK_SHOT);
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
			return;
		}
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	for (i = 0; i < 10; i++) {
		G_trap_Trace (&tr, tr_start, NULL, NULL, tr_end, passent, MASK_SHOT);
		traceEnt = &g_entities[ tr.entityNum ];
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	passent = ent->s.number;
	do {
		G_trap_Trace (&trace, muzzle, NULL, NULL, end, passent, MASK_SHOT );
	...
	for ( i = 0 ; i < unlinked ; i++ ) {
		G_trap_LinkEntity( unlinkedEntities[i] );
	}
...
static void KamikazeRadiusDamage( vec3_t origin, gentity_t *attacker, float damage, float radius ) {
	...
	numListedEntities = G_trap_EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES );

...
================
*/
intptr_t CG_vmMain( int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11  ) {
...
void QDECL CG_Com_Error( int level, const char *error, ... ) {
	va_list		argptr;
	char		text[1024];
...
void QDECL CG_Com_Printf( const char *msg, ... ) {
	va_list		argptr;
	char		text[1024];
...
//		}
	...
	cgs.media.youSuckSound = CG_trap_S_RegisterSound( "sound/misc/yousuck.wav", qfalse );
...
char *CG_GetMenuBuffer(const char *filename) {
	...
	if ( !f ) {
		CG_trap_Print( va( S_COLOR_RED "menu file not found: %s, using default\n", filename ) );
		return NULL;
	}
	if ( len >= MAX_MENUFILE ) {
		CG_trap_Print( va( S_COLOR_RED "menu file too large: %s is %i, max allowed is %i", filename, len, MAX_MENUFILE ) );
...
qboolean CG_Asset_Parse(int handle) {
	...
	if (!CG_trap_PC_ReadToken(handle, &token))
		return qfalse;
...
void CG_ParseMenu(const char *menuFile) {
	...
	handle = CG_trap_PC_LoadSource(menuFile);
	if (!handle)
		handle = CG_trap_PC_LoadSource("ui/testhud.menu");
	if (!handle)
		return;
	...
	while ( 1 ) {
		...
		//if ( Q_stricmp( token, "{" ) ) {
		//	CG_Com_Printf( "Missing { in menu file\n" );
		//	break;
		//}

		//if ( menuCount == MAX_MENUS ) {
		//	CG_Com_Printf( "Too many menus!\n" );
		//	break;
		//}
...
void CG_LoadMenus(const char *menuFile) {
	...
	if ( !f ) {
		CG_trap_Error( va( S_COLOR_YELLOW "menu file not found: %s, using default\n", menuFile ) );
		len = CG_trap_FS_FOpenFile( "ui/hud.txt", &f, FS_READ );
		if (!f) {
			CG_trap_Error( va( S_COLOR_RED "default menu file not found: ui/hud.txt, unable to continue!\n") );
		}
	}

	if ( len >= MAX_MENUDEFFILE ) {
		CG_trap_Error( va( S_COLOR_RED "menu file too large: %s is %i, max allowed is %i", menuFile, len, MAX_MENUDEFFILE ) );
		CG_trap_FS_FCloseFile( f );
		return;
	}
	...
	CG_Com_Printf("UI menu load time = %d milli seconds\n", CG_trap_Milliseconds() - start);
...
void CG_LoadHudMenu( void ) {
	...
	//cgDC.setOverstrikeMode = &CG_trap_Key_SetOverstrikeMode;
	//cgDC.getOverstrikeMode = &CG_trap_Key_GetOverstrikeMode;
	...
	//cgDC.setBinding = &CG_trap_Key_SetBinding;
	//cgDC.getBindingBuf = &CG_trap_Key_GetBindingBuf;
	//cgDC.keynumToStringBuf = &CG_trap_Key_KeynumToStringBuf;
	//cgDC.executeText = &CG_trap_Cmd_ExecuteText;
	cgDC.Error = &CG_Com_Error; 
	cgDC.Print = &CG_Com_Printf; 
...
void CG_AssetCache( void ) {
	...
	//CG_Com_Printf("Menu Size: %i bytes\n", sizeof(Menus));

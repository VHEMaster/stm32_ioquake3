...
void CG_dllEntry( intptr_t (QDECL  *syscallptr)( intptr_t arg,... ) ) {
  syscall = syscallptr;
}
...
void  CG_trap_Print( const char *fmt ) {
  syscall( CG_PRINT, fmt );
}

void  CG_trap_Error( const char *fmt ) {
...
int CG_trap_FS_Seek( fileHandle_t f, long offset, int origin ) {
...
clipHandle_t CG_trap_CM_InlineModel( int index ) {
...
clipHandle_t CG_trap_CM_TempCapsuleModel( const vec3_t mins, const vec3_t maxs ) {
...
void  CG_trap_S_StopLoopingSound( int entityNum ) {
...
sfxHandle_t CG_trap_S_RegisterSound( const char *sample, qboolean compressed ) {
  return syscall( CG_S_REGISTERSOUND, sample, compressed );
}

void  CG_trap_S_StartBackgroundTrack( const char *intro, const char *loop ) {
...
qhandle_t CG_trap_R_RegisterModel( const char *name ) {
  return syscall( CG_R_REGISTERMODEL, name );
}

qhandle_t CG_trap_R_RegisterSkin( const char *name ) {
  return syscall( CG_R_REGISTERSKIN, name );
}

qhandle_t CG_trap_R_RegisterShader( const char *name ) {
  return syscall( CG_R_REGISTERSHADER, name );
}

qhandle_t CG_trap_R_RegisterShaderNoMip( const char *name ) {
...
void  CG_trap_R_AddPolysToScene( qhandle_t hShader , int numVerts, const polyVert_t *verts, int num ) {
...
qboolean CG_trap_Key_IsDown( int keynum ) {
  return syscall( CG_KEY_ISDOWN, keynum );
}

int CG_trap_Key_GetCatcher( void ) {
  return syscall( CG_KEY_GETCATCHER );
}

void CG_trap_Key_SetCatcher( int catcher ) {
  syscall( CG_KEY_SETCATCHER, catcher );
}

int CG_trap_Key_GetKey( const char *binding ) {
  return syscall( CG_KEY_GETKEY, binding );
}

int CG_trap_PC_AddGlobalDefine( char *define ) {
  return syscall( CG_PC_ADD_GLOBAL_DEFINE, define );
}

int CG_trap_PC_LoadSource( const char *filename ) {
  return syscall( CG_PC_LOAD_SOURCE, filename );
}

int CG_trap_PC_FreeSource( int handle ) {
  return syscall( CG_PC_FREE_SOURCE, handle );
}

int CG_trap_PC_ReadToken( int handle, pc_token_t *pc_token ) {
  return syscall( CG_PC_READ_TOKEN, handle, pc_token );
}

int CG_trap_PC_SourceFileAndLine( int handle, char *filename, int *line ) {
...
/*
qboolean CG_trap_loadCamera( const char *name ) {
  return syscall( CG_LOADCAMERA, name );
}

void CG_trap_startCamera(int time) {
  syscall(CG_STARTCAMERA, time);
}

qboolean CG_trap_getCameraInfo( int time, vec3_t *origin, vec3_t *angles) {

...
void CG_ParseServerinfo( void ) {
	...
	CG_trap_Cvar_Set("g_gametype", va("%i", cgs.gametype));
	...
	CG_trap_Cvar_Set("g_redTeam", cgs.redTeam);
	Q_strncpyz( cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam) );
	CG_trap_Cvar_Set("g_blueTeam", cgs.blueTeam);
}
...
static void CG_ParseWarmup( void ) {
	...
	if ( warmup == 0 && cg.warmup ) {
		...
		if (cgs.gametype >= GT_CTF && cgs.gametype <= GT_HARVESTER) {
			CG_trap_S_StartLocalSound( cgs.media.countPrepareTeamSound, CHAN_ANNOUNCER );
		} else
#endif
		{
			CG_trap_S_StartLocalSound( cgs.media.countPrepareSound, CHAN_ANNOUNCER );
		}
	}
	...
	while (o && *o) {
		...
		if (n && *n) {
			...
			if (o) {
				...
				timeOffset[o-t] = 0;
				o++;
				CG_trap_R_RemapShader( originalShader, newShader, timeOffset );
			}
		} else {
			break;
		}
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
...
#ifdef MISSIONPACK
		CG_trap_S_StartLocalSound( cgs.media.voteNow, CHAN_ANNOUNCER );
#endif //MISSIONPACK
		...
		cgs.gameModels[ num-CS_MODELS ] = CG_trap_R_RegisterModel( str );
	} else if ( num >= CS_SOUNDS && num < CS_SOUNDS+MAX_SOUNDS ) {
		if ( str[0] != '*' ) {	// player specific sounds don't register here
			cgs.gameSounds[ num-CS_SOUNDS] = CG_trap_S_RegisterSound( str, qfalse );
		}
...
static void CG_MapRestart( void ) {
	...
	if ( cg.warmup == 0 /* && cgs.gametype == GT_TOURNAMENT */) {
		CG_trap_S_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
		CG_CenterPrint( "FIGHT!", 120, GIANTCHAR_WIDTH*2 );
	}
#ifdef MISSIONPACK
	if (cg_singlePlayerActive.integer) {
		CG_trap_Cvar_Set("ui_matchStartTime", va("%i", cg.time));
		if (cg_recordSPDemo.integer && cg_recordSPDemoName.string && *cg_recordSPDemoName.string) {
			CG_trap_SendConsoleCommand(va("set g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
		}
	}
#endif
...
int CG_ParseVoiceChats( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats ) {
	...
	if ( !f ) {
		CG_trap_Print( va( S_COLOR_RED "voice chat file not found: %s\n", filename ) );
		return qfalse;
	}
	if ( len >= MAX_VOICEFILESIZE ) {
		CG_trap_Print( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len, MAX_VOICEFILESIZE ) );
	...
	if (!Q_stricmp(token, "female")) {
		...
		CG_trap_Print( va( S_COLOR_RED "expected gender not found in voice chat file: %s\n", filename ) );
		return qfalse;
	}
	...
	while ( 1 ) {
		...
		token = COM_ParseExt(p, qtrue);
		if (Q_stricmp(token, "{")) {
			CG_trap_Print( va( S_COLOR_RED "expected { found %s in voice chat file: %s\n", token, filename ) );
			return qfalse;
		}
		...
		while(1) {
			...
			if (!Q_stricmp(token, "}"))
				break;
			sound = CG_trap_S_RegisterSound( token, compress );
...
int CG_HeadModelVoiceChats( char *filename ) {
	...
	len = CG_trap_FS_FOpenFile( filename, &f, FS_READ );
	if ( !f ) {
		//CG_trap_Print( va( "voice chat file not found: %s\n", filename ) );
		return -1;
	}
	if ( len >= MAX_VOICEFILESIZE ) {
		CG_trap_Print( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len, MAX_VOICEFILESIZE ) );
...
void CG_PlayVoiceChat( bufferedVoiceChat_t *vchat ) {
	...
	if ( !cg_noVoiceChats.integer ) {
		CG_trap_S_StartLocalSound( vchat->snd, CHAN_VOICE);
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "print" ) ) {
		...
		if ( !Q_stricmpn( cmd, "vote failed", 11 ) || !Q_stricmpn( cmd, "team vote failed", 16 )) {
			CG_trap_S_StartLocalSound( cgs.media.voteFailed, CHAN_ANNOUNCER );
		} else if ( !Q_stricmpn( cmd, "vote passed", 11 ) || !Q_stricmpn( cmd, "team vote passed", 16 ) ) {
			CG_trap_S_StartLocalSound( cgs.media.votePassed, CHAN_ANNOUNCER );
		}
#endif
		return;
	}
	...
	if ( Q_stricmp (cmd, "remapShader") == 0 )
	{
		if (CG_trap_Argc() == 4)
		{
			...
			CG_trap_R_RemapShader(shader1, shader2, shader3);
		}
...
void CG_ExecuteNewServerCommands( int latestSequence ) {
	while ( cgs.serverCommandSequence < latestSequence ) {
		if ( CG_trap_GetServerCommand( ++cgs.serverCommandSequence ) ) {
			CG_ServerCommand();
		}
	}
}

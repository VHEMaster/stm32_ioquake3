...
void Q3UI_SPArena_Start( const char *arenaInfo ) {
	...
	n = (int)UI_trap_Cvar_VariableValue( "sv_maxclients" );
	if ( n < 8 ) {
		UI_trap_Cvar_SetValue( "sv_maxclients", 8 );
	}
	...
	if( txt[0] ) {
		if( Q_stricmp( txt, "training" ) == 0 ) {
			...
			level = Q3UI_GetNumSPTiers() * ARENAS_PER_TIER;
		}
	}
	UI_trap_Cvar_SetValue( "ui_spSelection", level );

	map = Info_ValueForKey( arenaInfo, "map" );
	UI_trap_Cmd_ExecuteText( EXEC_APPEND, va( "spmap %s\n", map ) );
}

...
static fileHandle_t FS_HandleForFile(void) {
  ...
  for ( i = 1 ; i < MAX_FILE_HANDLES ; i++ ) {
    if ( fsh[i].handleFiles.file.o.obj.fs == NULL ) {
      return i;
    }
  }
...
static FIL* FS_FileForHandle( fileHandle_t f ) {
  if ( f < 0 || f > MAX_FILE_HANDLES ) {
    Com_Error( ERR_DROP, "FS_FileForHandle: out of range" );
  }
  if (fsh[f].zipFile == qtrue) {
    Com_Error( ERR_DROP, "FS_FileForHandle: can't get FIL on zip file" );
  }
  if ( ! fsh[f].handleFiles.file.o.obj.fs ) {
...
If this is called on a non-unique FIL (from a pak file),
...
int FS_filelength( fileHandle_t f ) {
  ...
  size = f_size(&fsh[f].handleFiles.file.o);
...
static void FS_CopyFile( char *fromOSPath, char *toOSPath ) {
  ...
  res = f_open(&f, fromOSPath, FA_READ );
  if ( res != FR_OK ) {
    return;
  }
  len = f_size(&f);
  ...
  res = f_read(&f, buf, len, &ctrl);
  if (res != FR_OK || len != ctrl)
  ...
  res = f_open(&f, toOSPath, FA_WRITE | FA_CREATE_ALWAYS );
  if (res != FR_OK) {
    return;
  }
  res = f_write(&f, buf, len, &ctrl);
  if (res != FR_OK || ctrl != len)
...
void FS_HomeRemove( const char *homePath ) {
  ...
  f_unlink( FS_BuildOSPath( fs_homepath->string,
      fs_gamedir, homePath ) );
}
...
fileHandle_t FS_SV_FOpenFileWrite( const char *filename ) {
  ...
  res = f_open(&fsh[f].handleFiles.file.o, ospath, FA_WRITE | FA_CREATE_ALWAYS );
...
int FS_SV_FOpenFileRead( const char *filename, fileHandle_t *fp ) {
  ...
  res = f_open(&fsh[f].handleFiles.file.o, ospath, FA_READ );
  fsh[f].handleSync = qfalse;
  if (res != FR_OK)
  {
...
void FS_SV_Rename( const char *from, const char *to ) {
  ...
  if (f_rename( from_ospath, to_ospath ) != FR_OK) {
...
If the FIL pointer is an open pak file, leave it open.

For some reason, other dll's can't just cal f_close()
...
void FS_FCloseFile( fileHandle_t f ) {
  ...
  f_close (&fsh[f].handleFiles.file.o);
  Com_Memset( &fsh[f], 0, sizeof( fsh[f] ) );
}
...
fileHandle_t FS_FOpenFileAppend( const char *filename ) {
  ...
  res = f_open(&fsh[f].handleFiles.file.o, ospath, FA_WRITE | FA_OPEN_APPEND);
  fsh[f].handleSync = qfalse;
  if (res != FR_OK) {
    f = 0;
  }
...
Returns filesize and an open FIL pointer.
  ...
  if ( file == NULL ) {
    ...
    for ( search = fs_searchpaths ; search ; search = search->next ) {
      ...
      if ( search->pack && search->pack->hashTable[hash] ) {
        ...
        res = f_open (&fil, netpath, FA_READ);
        if (res != FR_OK) {
          continue;
        }
        f_close(&fil);
  ...
  *file = FS_HandleForFile();
  ...
  for ( search = fs_searchpaths ; search ; search = search->next ) {
    ...
    if ( search->pack && search->pack->hashTable[hash] ) {
      ...
      netpath = FS_BuildOSPath( dir->path, dir->gamedir, filename );
      res = f_open (&fsh[*file].handleFiles.file.o, netpath, FA_READ);
      if (res != FR_OK) {
        continue;
      }
...
int FS_Read( void *buffer, int len, fileHandle_t f ) {
  ...
  if (fsh[f].zipFile == qfalse) {
    ...
    while (remaining) {
      block = remaining;
      res = f_read (&fsh[f].handleFiles.file.o, buf, block, &read);
      if (res != FR_OK || read == 0) {
...
int FS_Write( const void *buffer, int len, fileHandle_t h ) {
  ...
  while (remaining) {
    block = remaining;
    res = f_write (f, buf, block, &written);
...
int FS_Seek( fileHandle_t f, long offset, int origin ) {
  ...
  if (fsh[f].zipFile == qtrue) {
    ...
    return f_lseek( file, offset + _origin );
  }
}
    ...
    if (Q_stricmp(name, BASEGAME) && Q_stricmpn(name, ".", 1)) {
      ...
      if (nPaks > 0) {
        ...
        if ( nDescLen > 0 && descHandle) {
          ...
          res = f_read(file, descPath, 48, &nDescLen);
          if (res == FR_OK && nDescLen >= 0) {
            descPath[nDescLen] = '\0';
          }
...
static void FS_Startup( const char *gameName )
{
  ...
  if (missingFiles == NULL) {
    missingFiles = f_open( "\\missing.txt", "ab" );
  }
#endif
...
int   FS_FOpenFileByMode( const char *qpath, fileHandle_t *f, fsMode_t mode ) {
  ...
  if ( *f ) {
    if (fsh[*f].zipFile == qtrue) {
      ...
      fsh[*f].baseOffset = f_tell(&fsh[*f].handleFiles.file.o);
    }
    fsh[*f].fileSize = r;
    fsh[*f].streamed = qfalse;
...
int   FS_FTell( fileHandle_t f ) {
  ...
  if (fsh[f].zipFile == qtrue) {
    ...
    pos = f_tell(&fsh[f].handleFiles.file.o);
  }
  return pos;
}

void  FS_Flush( fileHandle_t f ) {
  f_sync(&fsh[f].handleFiles.file.o);
}

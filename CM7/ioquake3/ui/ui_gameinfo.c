...
int UI_ParseInfos( char *buf, int max, char *infos[] ) {
	...
	while ( 1 ) {
		...
		if ( strcmp( token, "{" ) ) {
			UI_Com_Printf( "Missing { in info file\n" );
			break;
		}

		if ( count == max ) {
			UI_Com_Printf( "Max infos exceeded\n" );
			break;
		}
		...
		while ( 1 ) {
			...
			if ( !token[0] ) {
				UI_Com_Printf( "Unexpected end of info file\n" );
				break;
			}
			if ( !strcmp( token, "}" ) ) {
				break;
...
void UI_LoadArenas( void ) {
	...
	if (UI_OutOfMemory()) {
		UI_trap_Print(S_COLOR_YELLOW"WARNING: not anough memory in pool to load all arenas\n");
	}

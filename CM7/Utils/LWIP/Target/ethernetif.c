...
ETH_DMADescTypeDef DMARxDscrTab[ETH_RX_DESC_CNT] __attribute__((section(".AxiRamSection"))); /* Ethernet Rx DMA Descriptors */
ETH_DMADescTypeDef DMATxDscrTab[ETH_TX_DESC_CNT] __attribute__((section(".AxiRamSection")));   /* Ethernet Tx DMA Descriptors */
uint8_t Rx_Buff[ETH_RX_DESC_CNT][ETH_RX_BUFFER_SIZE] __attribute__((section(".AxiRamSection"))); /* Ethernet Receive Buffers */
...
osSemaphoreId TxPktSemaphore = NULL; /* Semaphore to signal outcoming packets */
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != NULL; q = q->next)
  {
    ...
    //SCB_CleanDCache_by_Addr((uint32_t *)((uint32_t)q->payload & ~0x1F), q->len & ~0x1F + 0x20);
    i++;
  }
  ...
  //osSemaphoreAcquire(TxPktSemaphore, ETH_DMA_TRANSMIT_TIMEOUT);
...
static struct pbuf * low_level_input(struct netif *netif)
{
  ...
  if (HAL_ETH_GetRxDataBuffer(&heth, &RxBuff) == HAL_OK) 
  {
    ...
    //SCB_InvalidateDCache_by_Addr((uint32_t *)((uint32_t)RxBuff.buffer & ~0x1F), framelength & ~0x1F + 0x20);
#endif
